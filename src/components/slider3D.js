 ...
 * - Flagship Project Highlight Badging
...
export function init3DSlider(onProjectSelect) {
  ...
  track.innerHTML = projects
    .map(
      (p, idx) => `
    <div class="slider-3d-card ${p.isFlagship ? 'is-flagship-card' : ''}" data-index="${idx}" data-project-id="${p.id}">
      <div class="card-inner-shell">
        <div class="card-visual-header">
          ...
          <div class="card-badge-row">
            ...
            ${p.isFlagship ? '<span class="card-flagship-badge">★ MAIN FLAGSHIP</span>' : ''}
          </div>
        ...
        <div class="card-content-pane">
          <div class="card-meta-top">
            <span class="card-metric-tag ${p.isFlagship ? 'is-flagship-metric' : ''}">
          ...
          <div class="card-tags-list">
            ${p.tags.map((t) => `<span class="card-tag ${t.includes('★') ? 'flagship-tag' : ''}">${t}</span>`).join('')}
          </div>
          <div class="card-action-bar">
            <button class="card-inspect-btn ${p.isFlagship ? 'btn-flagship-inspect' : ''}" type="button" data-project-id="${p.id}">
  ...
  function updateSlider() {
    ...
    cards.forEach((card, idx) => {
      ...
      if (diff === 0) {
        ...
        const scale = isMobile ? '0.91' : '0.88';
        card.style.transform = `translateX(${xOffset}) scale(${scale})`;
        ...
        const scale = isMobile ? '0.78' : '0.75';
        card.style.transform = `translateX(${xOffset}) scale(${scale})`;
